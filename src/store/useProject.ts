			...
			(set) => ({
				...
				removeTask: (
					...
					set((state) => {
						...
						if (!project) return state;
						project.tasks[status] = project.tasks[status].filter(
				...
				updateTask: (
					...
					set((state) => {
						...
						if (!project) return state;
						project.tasks[status] = project.tasks[status].map((t) =>
				...
				updateTaskStatus: (
					...
					set((state) => {
						...
						if (!project) return state;
						project.tasks[statusType] = project.tasks[statusType].map(
							(task) => {
								if (task._id === taskId) {
...
export const useTask = () => {
	const { addTask, removeTask, updateTask, updateTaskStatus } = useProject(
		(state) => ({
